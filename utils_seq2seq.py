...
def truncate_tokens_signle(tokens_a, max_len, flag_c=False):
    if not flag_c:
...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def read_data(self, line, tokenizer):
        ...
        ctrl_tk = sample["ctrl_text"]
        return src_tk, tgt_tk, ctrl_tk
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __init__(self, mask_prob, vocab_words, indexer,
                 max_passage_length=20, max_tgt_length=66, max_ctrl_length=7, skipgram_prb=0, skipgram_size=0,
                 mask_whole_word=False, mask_source_words=True, tokenizer=None):
        super().__init__()
        self.max_a_len = max_passage_length
        self.max_b_len = max_tgt_length
        self.max_c_len = max_ctrl_length
        self.max_len = max_ctrl_length + max_passage_length + max_tgt_length + 1
        self.max_pred = self.max_len  # max tokens of prediction
    ...
    def __call__(self, instance):
        ...
        tokens_a, tokens_b, tokens_c = instance[:3]
        tokens_a = self.tokenizer.tokenize(tokens_a)  # 引导文本
        tokens_b = self.tokenizer.tokenize(tokens_b)  # 生成文本
        tokens_c = self.tokenizer.tokenize(tokens_c)  # 控制文本
        # -3  for special tokens [CLS], [SEP], [SEP]
        tokens_c = truncate_tokens_signle(tokens_c, self.max_c_len, True)
        tokens_a = truncate_tokens_signle(tokens_a, self.max_a_len)
        tokens_b = truncate_tokens_signle(tokens_b, self.max_b_len)
        # Add Special Tokens
        tokens_a = tokens_c + tokens_a
        tokens = ['[CLS]'] + tokens_a + ['[SEP]'] + tokens_b + ['[SEP]']
        segment_ids = [0] * (len(tokens_a) + 2) + [1] * (len(tokens_b) + 1)
        # c_tag = [0] * (len(tokens_a) + len(tokens_b) + 3)
        ...
        ctrl_ids = self.indexer(tokens_c)

        # Zero Padding
        n_pad = self.max_len - len(input_ids)
        c_n_pad = self.max_c_len - len(ctrl_ids)
        input_ids.extend([0] * n_pad)
        segment_ids.extend([0] * n_pad)
        ctrl_ids.extend([0] * c_n_pad)
        # label_tag.extend([0] * n_pad)
        ...
        return (input_ids, segment_ids, ctrl_ids, input_mask,
                masked_ids, masked_pos, masked_weights, next_sentence_label)
...
class Preprocess4Seq2seqDecode(Pipeline):
    ...
    def __init__(self, vocab_words, indexer, max_passage_length=20, max_tgt_length=66, max_ctrl_length=7, tokenizer=None):
        super().__init__()
        self.max_a_len = max_passage_length
        self.max_b_len = max_tgt_length
        self.max_c_len = max_ctrl_length
    ...
    def __call__(self, instance):
        ...
        tokens_a = tokens_c + tokens_a
        tokens = ['[CLS]'] + tokens_a + ['[SEP]']
        segment_ids = [0] * (self.max_a_len + self.max_c_len - 2)

        position_ids = []
        for i in range(len(tokens_a) + 2):
            position_ids.append(i)
        for i in range(len(tokens_a) + 2, self.max_a_len + self.max_c_len - 2):
            position_ids.append(0)
        for i in range(self.max_a_len + self.max_c_len - 2, self.max_len):
            position_ids.append(i - (self.max_a_len + self.max_c_len - 4 - len(tokens_a)))
        ...
        # Zero Padding
        n_pad = self.max_a_len + self.max_c_len - 2 - len(input_ids)
        input_ids.extend([0] * n_pad)
        c_n_pad = self.max_c_len - len(ctrl_ids)
        s_n_pad = self.max_len - len(input_ids)
        segment_ids.extend([1] * s_n_pad)
        ctrl_ids.extend([0] * c_n_pad)

        input_mask = torch.zeros(self.max_len, self.max_len, dtype=torch.long)
        input_mask[:, :self.max_a_len + self.max_c_len - 2].fill_(1)
        second_st, second_end = self.max_a_len + self.max_c_len - 2, self.max_len
        try:
            input_mask[second_st:second_end, second_st:second_end].copy_(
                self._tril_matrix[:second_end - second_st, :second_end - second_st])
        ...
        return input_ids, segment_ids, ctrl_ids, position_ids, input_mask
