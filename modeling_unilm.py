...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, embedding_ctrl=None, history_states=None):
        attention_output = self.attention(hidden_states, attention_mask, history_states=history_states)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output, embedding_ctrl)
        return layer_output
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, embedding_ctrl=None,
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                hidden_states = layer_module(hidden_states, attention_mask, embedding_ctrl,
...
class UnilmPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, BertOutput):
            module.beta_dense.weight.data.zero_()
            module.gamma_dense.weight.data.zero_()
...
class UnilmModel(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, ctrl_ids=None, attention_mask=None,
                output_all_encoded_layers=True):
        extended_attention_mask = self.get_extended_attention_mask(input_ids, token_type_ids, attention_mask)
        embedding_ctrl = self.embeddings(ctrl_ids)
        embedding_output = self.embeddings(input_ids, token_type_ids)
        encoded_layers = self.encoder(embedding_output, extended_attention_mask, embedding_ctrl,
...
class UnilmModelIncr(UnilmModel):
    ...
    def forward(self, input_ids, token_type_ids=None, ctrl_ids=None, position_ids=None, attention_mask=None,
                prev_embedding=None, prev_encoded_layers=None, output_all_encoded_layers=True):
        extended_attention_mask = self.get_extended_attention_mask(input_ids, token_type_ids, attention_mask)
        embedding_ctrl = self.embeddings(ctrl_ids)  # [12, 7, 768]
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, ctrl_ids=None, attention_mask=None,
                masked_lm_labels=None, masked_pos=None, masked_weights=None):
        sequence_output, __ = self.bert(input_ids, token_type_ids, ctrl_ids, attention_mask,
        ...
        def gather_seq_out_by_pos_average(seq, pos, mask):
            batch_size, max_token_num = pos.size(0), pos.size(-1)
            pos_vec = torch.gather(seq, 1, pos.view(batch_size, -1).unsqueeze(2).expand(-1, -1, seq.size(-1))).view(
                batch_size, -1, max_token_num, seq.size(-1))
            mask = mask.type_as(pos_vec)
        ...
        prediction_scores_masked = self.cls(sequence_output_masked)
        if self.crit_mask_lm_smoothed:
            masked_lm_loss = self.crit_mask_lm_smoothed(F.log_softmax(prediction_scores_masked.float(), dim=-1),
                                                        masked_lm_labels)
        else:
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    def __init__(self, config, mask_word_id=0,
                 ...
                 forbid_duplicate_ngrams=False, forbid_ignore_set=None, ngram_size=2, min_len=24):
        super(UnilmForSeq2SeqDecode, self).__init__(config)
        self.bert = UnilmModelIncr(config)
        self.cls = BertOnlyMLMHead(config)
        self.crit_mask_lm = nn.CrossEntropyLoss(ignore_index=0, reduction='none')
    ...
    def forward(self, input_ids, token_type_ids, ctrl_ids, position_ids, attention_mask):
        # if self.search_beam_size > 1:
        #     return self.beam_search(input_ids, token_type_ids, ctrl_ids, attention_mask)
        import time
        time1 = time.time()
        ...
        while next_pos < output_length:
            ...
            new_embedding, new_encoded_layers, _ = \
                self.bert(x_input_ids, curr_token_type_ids, ctrl_ids, curr_position_ids, curr_attention_mask,
    ...
    def beam_search(self, input_ids, token_type_ids, ctrl_ids, position_ids, attention_mask):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        ...
        while next_pos < output_length:
            ...
            if self.forbid_duplicate_ngrams:
                ...
                if is_first:
                    ...
                    for b in range(batch_size):
                        for k in range(K):
                            new_partial_seqs.append(partial_seqs[int(ptrs[b][k]) + b * K] + [wids[b][k]])
...
class UnilmForSeq2SeqDecodeSample(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids=None, token_type_ids=None, ctrl_ids=None, attention_mask=None):
        sequence_output, __ = self.bert(input_ids, token_type_ids, ctrl_ids, attention_mask)
